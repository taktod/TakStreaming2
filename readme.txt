...
アイデア
受け流しとかを有効にするために、ちょっとクラスの構成を変更したいと思います。

streamクラスはつぎのようにします。

TakStream (内部のコアな部分の動作)
↓
ShareStream (データを共有するための動作:自分の子にデータをおくる)
↓
RtmfpStream (p2p経由でデータをうけとる動作)
↓
HttpStream, RtmpStream, LocalFlvStream(サーバーからデータをうけとる動作)