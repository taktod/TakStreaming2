...
import com.ttProject.red5.CodecType;
...
//				System.out.println(Utils.toHex(data, true));
	...
	protected Boolean analizeAudioData(ByteBuffer buffer) {
		...
		if(headerPacket.getVideoCodec() == CodecType.NONE && !isSequenceHeader) {
			...
			if(passedTime >= TakApplicationAdapter.getDuration()) {
	...
	public ByteBuffer getBufferData(int number) {
		ByteBuffer buffer = getBuffer(0);
		buffer.flip();
		ByteBuffer result = ByteBuffer.allocate(buffer.remaining() + 12);
		// size
